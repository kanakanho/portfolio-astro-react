...
これだけではブログを投稿して、ソートなどの管理をするために十分な機能が提供できません。
...
無理をすれば Astro でページをレンダリングする際に markdown をパースして解析もできるのですが、それだと GitHub にコミットされた markdown がそのままでは利用できないため 利便性が下がります。
...
ライブラリの中に esa の webhook の POST リクエストの型があるので似たようなことをしたい人の参考になると思います。
